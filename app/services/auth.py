...
class AuthService:
    ...
    async def register_user(user_data: UserCreate) -> Dict[str, Any]:
        ...
        user_dict["is_author"] = False

        # Check if this email is in the author whitelist at registration
        whitelist_check = supabase.table("author_whitelist").select("id").eq(
            "email", user_data.email.lower().strip()
        ).execute()
        if whitelist_check.data:
            user_dict["is_author"] = True
    ...
    async def login_user(login_data: UserLogin) -> Token:
        ...
        # Check both primary email and secondary_email against whitelist
        if not user.get("is_author"):
            email = user["email"].lower().strip()
            secondary = (user.get("secondary_email") or "").lower().strip()

            whitelist_hit = supabase.table("author_whitelist").select("id").or_(
                f"email.eq.{email}" + (f",email.eq.{secondary}" if secondary else "")
            ).execute()

            if whitelist_hit.data:
        ...
        token_data = {
            ...
            "email": user["email"],
            "role": user["role"],
            "is_author": user.get("is_author", False),
        }
